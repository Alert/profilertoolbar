...
class Kohana_ProfilerToolbar {
  ...
  public static function firebug() {
    ...
    $titleStr = 'ProfilerToolbar v' . self::$version . ' for Kohana v' . self::$kohana_version;
    $titleStr = str_repeat('=', 10) . $titleStr . str_repeat('=', 10);
    self::$_fb->info($titleStr);
    // append other data
    ...
    // end
    self::$_fb->log(str_repeat('=', strlen($titleStr)));
  }
...
//    return $tmp['current']['memory'];
  ...
  private static function getSql() {
    ...
    foreach ($groups as $groupName => $benchmarks) {
      ...
      $sqlGroup = substr(
        $groupName,
        strpos($groupName, '(') + 1,
        strpos($groupName, ')') - strpos($groupName, '(') - 1
      );
  ...
  private static function appendTotalInfo() {
    $timeStr   = self::formatTime(self::$DATA_APP_TIME);
    $memoryStr = self::formatMemory(self::$DATA_APP_MEMORY);

    self::$_fb->log("[AppTime: {$timeStr}] [AppMemory: {$memoryStr}]");
  }
  ...
  private static function appendSql() {
    ...
    $timeStr   = self::formatTime($sql['time']);
    $memoryStr = self::formatMemory($sql['memory']);
    self::$_fb->group(
      "SQL [count: {$sql['count']}] [time: {$timeStr}] [memory: {$memoryStr}]",
    ...
    foreach (self::$DATA_SQL as $inst) {
      foreach ($inst['data'] as $q) {
        ...
        if (!empty($q['explain'])) {
          $tbl_explain[$num] = array(
            0 => array('id', 'select_type', 'table', 'type', 'possible_keys', 'key', 'key_len', 'ref', 'rows', 'Extra')
          );
  ...
  private static function appendCache() {
    $tGet = self::$DATA_CACHE['total']['get'];
    $tSet = self::$DATA_CACHE['total']['set'];
    $tDel = self::$DATA_CACHE['total']['del'];
    self::$_fb->group("Cache [GET: {$tGet}] [SET: {$tSet}] [DEL: {$tDel}]", array('Collapsed' => true));
  ...
  private static function appendRoutes() {
    ...
    $useRoute    = $tbl[$useNum];
    $useRouteStr = "{$useRoute[1]} | controller: {$useRoute[2]} | action: {$useRoute[3]} | params: {$useRoute[4]}";
    self::$_fb->group(
      'Routes (' . self::$DATA_ROUTES['total']['count'] . ') ' . $useRouteStr,
      array('Collapsed' => true)
    );
  ...
  public static function cacheLog($action, $instance, $id, $lifetime = null) {
    ...
    if (!isset(self::$_CACHE['total'])) {
      self::$_CACHE['total'] = array('get' => 0, 'set' => 0, 'del' => 0);
    }

    if (!isset(self::$_CACHE['data'][$instance]['total'])) {
      self::$_CACHE['data'][$instance]['total'] = array('get' => 0, 'set' => 0, 'del' => 0);
    }
   ...
   * @return string
   */
  public static function debugSource($file, $line_number, $padding = 5, $highlight = false, $lang = '') {
    ...
    foreach ($source as $row) {
      ...
      $result .=  '<span class="line' . $hRow . '"><span class="num">' .
                  sprintf($format, $line) .
                  ' </span>' . $row . '</span>';
      $line++;
    }
